...
## 배포 메모
- **API 키 불필요.** 서버는 로컬 GTFS 파일만 사용(외부 유료 API 없음). 지도 타일·Leaflet 은 클라이언트가 OpenStreetMap 공개 타일에서 로드.
- **메모리**: 전국 데이터 적재 시 힙 실사용 ~2GB. `-Xmx4g` 권장(서버 RAM 4~6GB).
- **기동**: GTFS 적재 ~30초 + Timetable 빌드 수 초. 인메모리·무상태 → 수평 확장 용이(인스턴스별 적재).
- **응답**: 쿼리 ~100~200ms.
